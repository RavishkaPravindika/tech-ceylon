...
export default function CreateProductPage() {
  ...
  const { register, handleSubmit, formState: { errors }, control, watch, setValue } = useForm<ProductForm>({
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const urls = await Promise.all(
        files.map((file) => uploadProductImage(file, tempId))
      );
  ...
  const onSubmit = async (data: ProductForm) => {
    ...
    try {
      ...
      await createProduct(productData as any, firebaseUser.uid, firebaseUser.displayName || 'Admin');
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-4xl">
      {/* Back button */}
      <Link href={ROUTES.ADMIN_PRODUCTS} className="inline-flex items-center gap-2 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="sm:col-span-2">
            ...
            <input {...register('name')} id="product-name" placeholder="e.g. Wireless Mechanical Keyboard"
          ...
          {/* Product Code */}
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">Product Code *</label>
            <input {...register('productCode')} id="product-code" placeholder="e.g. TK-001"
          ...
          <div>
            ...
            <input {...register('brand')} id="product-brand" placeholder="e.g. Logitech"
          ...
          <div>
            ...
            <select {...register('categoryId')} id="product-category"
          ...
          <div>
            ...
            <select {...register('status')} id="product-status"
          ...
          {/* Price */}
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">Price (Rs.) *</label>
            <input {...register('price')} type="number" id="product-price" placeholder="0"
          ...
          {/* Discount Price */}
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">Discount Price (Rs.) — optional</label>
            <input {...register('discountPrice')} type="number" id="product-discount-price" placeholder="Leave empty if no discount"
              className="w-full px-4 py-2.5 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl text-sm text-[var(--text-primary)] focus:outline-none focus:border-blue-500" />
          </div>

          {/* Stock */}
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">Stock Quantity</label>
            <input {...register('stockQuantity')} type="number" id="product-stock" placeholder="0"
              className="w-full px-4 py-2.5 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl text-sm text-[var(--text-primary)] focus:outline-none focus:border-blue-500" />
          </div>

          {/* Featured */}
          <div className="flex items-center gap-3 pt-6">
            <input {...register('featured')} type="checkbox" id="product-featured" className="w-4 h-4 accent-blue-600" />
            <label htmlFor="product-featured" className="text-sm font-medium text-[var(--text-secondary)] cursor-pointer">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea {...register('shortDescription')} id="product-short-desc" rows={2} placeholder="Brief product summary"
          ...
          {/* Full Description */}
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">Full Description *</label>
            <textarea {...register('description')} id="product-desc" rows={5} placeholder="Detailed product description"
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          {images.map((url, idx) => (
            <div key={idx} className="relative w-24 h-24 rounded-xl overflow-hidden border border-[var(--border-color)]">
          ...
          <label className={`w-24 h-24 rounded-xl border-2 border-dashed border-[var(--border-color)] flex flex-col items-center justify-center cursor-pointer hover:border-blue-400 transition-colors ${uploadingImages ? 'opacity-50 cursor-not-allowed' : ''}`}>
            {uploadingImages ? (
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              ...
              if (val) { setValue('images', [...images, val]); (e.target as HTMLInputElement).value = ''; }
            }
          }}
        />
      </div>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={() => appendSpec({ key: '', value: '' })} className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-700">
        ...
        <div className="space-y-2">
          {specFields.map((field, idx) => (
            <div key={field.id} className="flex gap-2">
              <input {...register(`specifications.${idx}.key`)} placeholder="e.g. Connectivity" className="flex-1 px-3 py-2 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl text-sm focus:outline-none focus:border-blue-500" />
              <input {...register(`specifications.${idx}.value`)} placeholder="e.g. Bluetooth 5.0" className="flex-1 px-3 py-2 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl text-sm focus:outline-none focus:border-blue-500" />
              {specFields.length > 1 && (
                <button type="button" onClick={() => removeSpec(idx)} className="p-2 text-red-400 hover:text-red-600 transition-colors">
                  <Minus size={14} />
                </button>
              )}
            </div>
      ...
      <div className="flex gap-4 pb-8">
        <Link href={ROUTES.ADMIN_PRODUCTS} className="px-6 py-3 border border-[var(--border-color)] text-[var(--text-secondary)] rounded-xl text-sm font-medium hover:bg-[var(--bg-secondary)] transition-all">
          Cancel
        </Link>
        <button type="submit" id="create-product-submit" disabled={isSubmitting}
